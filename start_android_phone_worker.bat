...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$ok = Test-NetConnection 127.0.0.1 -Port 4723 -InformationLevel Quiet; if (-not $ok) { exit 1 }"
if errorlevel 1 (
  echo.
  ...
  echo [android-phone-worker] Appium is only required for inspect/unlink UI automation.
)
